...
let state = {
  ...
  editingEvent: null,  // id del evento que se está editando
...
function renderTimeline(wrap, family, meId, year) {
  ...
  if (yearEvents.length > 0) {
    ...
    yearEvents.forEach((ev) => {
      const isAutoEvent = ev.id && (ev.id.startsWith("b-") || ev.id.startsWith("d-"));
      ...
      card.innerHTML = `
        ...
        ${!isAutoEvent ? `<div class="event-actions"><button class="ev-edit" data-evid="${ev.id}">✏️ Editar</button><button class="ev-delete" data-evid="${ev.id}">🗑️ Borrar</button></div>` : ""}`;
      ...
      const editBtn = card.querySelector(".ev-edit");
      if (editBtn) editBtn.onclick = () => setState({ editingEvent: ev.id });
      const delBtn = card.querySelector(".ev-delete");
      if (delBtn) delBtn.onclick = () => {
        if (!confirm(`¿Eliminar el evento "${ev.title}"?`)) return;
        const updatedEvents = (state.family.events || []).filter((e) => e.id !== ev.id);
        const updatedFamily = { ...state.family, events: updatedEvents };
    ...
    if (state.editingEvent) {
      const evToEdit = (state.family.events || []).find((e) => e.id === state.editingEvent);
      if (evToEdit) {
        renderEditEventForm(wrap, evToEdit);
      }
    }
  }
...
function renderMap(wrap, family, meId, year) {
  ...
  mapContainer.style.cssText = "width:100%;height:320px;border-radius:16px;margin:0 0 12px;overflow:hidden;background:#e8e0d0";
  ...
  requestAnimationFrame(() => {
    ...
    // Tiles estilo claro tipo Apple Maps
...
function renderEditEventForm(wrap, ev) {
  ...
  const form = document.createElement("div"); form.className = "add-form";
  form.innerHTML = `
    <div class="form-title">Editar evento</div>
    <label>Año</label><input id="ee-year" type="number" value="${ev.year}" />
    <label>Título</label><input id="ee-title" value="${ev.title}" />
    <label>Descripción corta</label><input id="ee-desc" value="${ev.desc || ''}" />
    <label>Historia larga (opcional)</label>
    <textarea id="ee-story" rows="3" style="width:100%;box-sizing:border-box;background:var(--card);border:1px solid var(--line);border-radius:8px;padding:8px 11px;font-size:13px;color:var(--paper);font-family:inherit;resize:vertical;margin-bottom:10px">${ev.story || ''}</textarea>
    <select id="ee-cat">${CATS.map((c) => `<option value="${c.id}" ${ev.category === c.id ? "selected" : ""}>${c.label}</option>`).join("")}</select>
    <label>¿A quién afecta?</label>
    <select id="ee-members" multiple style="height:80px">${people.map((p) => `<option value="${p.id}" ${(ev.memberIds || []).includes(p.id) ? "selected" : ""}>${p.name}</option>`).join("")}</select>
  ...
  wrap.appendChild(form);
  form.querySelector("#ee-cancel").onclick = () => setState({ editingEvent: null });
  form.querySelector("#ee-save").onclick = () => {
    const year = parseInt(form.querySelector("#ee-year").value);
    const title = form.querySelector("#ee-title").value.trim();
    if (!year || !title) return;
    const updated = {
      ...ev,
      year, title,
      desc: form.querySelector("#ee-desc").value.trim(),
      story: form.querySelector("#ee-story").value.trim(),
      category: form.querySelector("#ee-cat").value,
      memberIds: Array.from(form.querySelector("#ee-members").selectedOptions).map((o) => o.value),
    };
    const updatedEvents = (state.family.events || []).map((e) => e.id === ev.id ? updated : e);
    const updatedFamily = { ...state.family, events: updatedEvents };
    setState({ family: updatedFamily, editingEvent: null });
    persistFamily();
  };
}
